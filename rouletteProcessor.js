...
class StealthRouletteProcessor extends EventEmitter {
  constructor(db) {
    ...
    this.config = {
      ...
      // GAME_TIMEOUT must be long enough for: bet delay (1500-3000ms) + result wait (~2500ms) + buffer
      // Min safe value = 3000 + 2500 + 2000 = 7500ms. Set to 12-18s to be safe.
  ...
  async stealthAccountFlow(account, index, sessionId) {
    ...
    // Login can take up to ~4000ms with slow proxies.
    // Game needs: bet delay(1500-3000ms) + result(~2500ms) + buffer(2000ms) = ~8000ms minimum.
    // So total must be >= 12000ms. Use 20-35s to be comfortable.
  ...
  async stealthGameFlow(account, userAgent, headers, proxy, index, sessionId, sessionStart, maxDuration) {
    return new Promise((resolve) => {
      ...
      // Game phase timeout:
      // MUST be long enough for bet delay (1500-3000ms) + server result (~2300ms) + buffer (2000ms).
      // Minimum safe = 8000ms. We use Math.MAX so we always get the LARGER of gameTimeout vs remainingTime.
      // This prevents the "session budget ran out before result arrived" loss bug.
      ...
      const mainTimeout = setTimeout(() => {
        if (!completed) {
          // If bet was never even sent → connection issue, not a game loss
          // If bet was sent but no result → server timeout (rare, not account's fault)
          this.log(index, 'warning', `⚠️ Game timeout after ${Math.round(effectiveTimeout/1000)}s — bet may not have landed`);
          finish({ 
      ...
      gameWs.on('open', () => {
        ...
        // Bet placed at random time (1.5-3 seconds after connection)
        // betDelay: 800-1800ms. Enough to look human but leaves plenty of time for result.
        // Old 1500-3000ms was burning session budget before result arrived.
